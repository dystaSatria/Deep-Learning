# DENSENET169_Optuna Hyperparameter

---

Bu detaylı dokümantasyon, DENSENET169 tabanlı görüntü sınıflandırma sisteminin tüm yönlerini kapsamaktadır. Kod'un her bölümü, kullanım senaryoları ve optimizasyon stratejileri açık bir şekilde açıklanmıştır.# DENSENET169 ile Gelişmiş Görüntü Sınıflandırma - Detaylı Kod Analizi

Bu projede, DENSENET169 önceden eğitilmiş modelini kullanarak kapsamlı bir görüntü sınıflandırma sistemi geliştirilmiştir. Bu doküman, kodun her bölümünü detaylı olarak açıklamaktadır.
...
5. [DENSENET169 Model Fonksiyonları](#DENSENET169-model-fonksiyonları)
...
```
[Görüntü Verisi] → [DENSENET169 Feature Extractor] → [ML Algoritmaları] → [Tahmin]
                          ↓
...
2. **DENSENET169 Eğitimi**: Transfer learning ile fine-tuning
...
```python
from tensorflow.keras.applications import DENSENET169  # Önceden eğitilmiş model
...
## 5. DENSENET169 Model Fonksiyonları

### 5.1 Feature Extractor Oluşturma

```python
def create_feature_extractor():
    # DENSENET169'i ImageNet ağırlıkları ile yükle
...
### 5.3 DENSENET169 Model Oluşturma
...
def objective_densenet(trial, data_folder):
    """Optuna objective function for optimizing DENSENET169 hyperparameters"""
...
def optimize_densenet(data_folder, n_trials=20):
    ...
    print(f"Starting DENSENET169 hyperparameter optimization with {n_trials} trials...")
...
def run_enhanced_classification_with_optuna(data_folder, run_optimization=True, n_optimization_trials=10):
    print(f"Starting enhanced DENSENET169 classification with Optuna optimization on {data_folder}...")
...
# 2. HİPERPARAMETRE OPTİMİZASYONU
    ...
    # 5. DENSENET169 MODEL EĞİTİMİ
    print("\n===== Training Optimized DENSENET169 Model =====")
    model = build_densenet_model(
        num_classes=num_classes,
        dense_neurons=dense_neurons,
        dropout_rate=dropout_rate,
    ...
    densenet_metrics['model'] = "DENSENET169 (Optimized)"
    ...
    all_models_results["DENSENET169 (Optimized)"] = {
        "y_true": y_true,
        "y_pred": y_pred,
        "y_prob": y_pred_prob
    }
    ...
    if run_optimization and 'svm' in best_params:
        ...
        all_models_results[f"DENSENET169 + SVM ({kernel})"] = {
            "y_true": test_labels,
            "y_pred": svm_pred,
            "y_prob": svm_prob
        }
        ...
        if svm_metrics['f1'] > best_hybrid_model_f1:
            ...
            best_hybrid_model_name = f"DENSENET169 + SVM ({kernel})"
    ...
    if best_hybrid_model_name:
        ...
        if densenet_f1 > best_hybrid_model_f1:
            print(f"Note: DENSENET169 (Optimized) still outperforms hybrid models with F1 score: {densenet_f1:.4f}")
        else:
            print(f"Hybrid model outperforms DENSENET169 (Optimized) (F1: {densenet_f1:.4f})")
    else:
