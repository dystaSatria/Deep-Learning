# DENSENET201_Optuna Hyperparameter

---

Bu detaylı dokümantasyon, DENSENET201 tabanlı görüntü sınıflandırma sisteminin tüm yönlerini kapsamaktadır. Kod'un her bölümü, kullanım senaryoları ve optimizasyon stratejileri açık bir şekilde açıklanmıştır.# DENSENET201 ile Gelişmiş Görüntü Sınıflandırma - Detaylı Kod Analizi

Bu projede, DENSENET201 önceden eğitilmiş modelini kullanarak kapsamlı bir görüntü sınıflandırma sistemi geliştirilmiştir. Bu doküman, kodun her bölümünü detaylı olarak açıklamaktadır.
...
5. [DENSENET201 Model Fonksiyonları](#DENSENET201-model-fonksiyonları)
...
```
[Görüntü Verisi] → [DENSENET201 Feature Extractor] → [ML Algoritmaları] → [Tahmin]
                          ↓
...
2. **DENSENET201 Eğitimi**: Transfer learning ile fine-tuning
...
```python
from tensorflow.keras.applications import DENSENET201  # Önceden eğitilmiş model
...
## 5. DENSENET201 Model Fonksiyonları

### 5.1 Feature Extractor Oluşturma

```python
def create_feature_extractor():
    # DENSENET201'i ImageNet ağırlıkları ile yükle
...
### 5.3 DENSENET201 Model Oluşturma
...
def objective_densenet(trial, data_folder):
    """Optuna objective function for optimizing DENSENET201 hyperparameters"""
...
def optimize_densenet(data_folder, n_trials=20):
    ...
    print(f"Starting DENSENET201 hyperparameter optimization with {n_trials} trials...")
...
def run_enhanced_classification_with_optuna(data_folder, run_optimization=True, n_optimization_trials=10):
    print(f"Starting enhanced DENSENET201 classification with Optuna optimization on {data_folder}...")
...
# 2. HİPERPARAMETRE OPTİMİZASYONU
    ...
    # 5. DENSENET201 MODEL EĞİTİMİ
    print("\n===== Training Optimized DENSENET201 Model =====")
    model = build_densenet_model(
        num_classes=num_classes,
        dense_neurons=dense_neurons,
        dropout_rate=dropout_rate,
    ...
    densenet_metrics['model'] = "DENSENET201 (Optimized)"
    ...
    all_models_results["DENSENET201 (Optimized)"] = {
        "y_true": y_true,
        "y_pred": y_pred,
        "y_prob": y_pred_prob
    }
    ...
    if run_optimization and 'svm' in best_params:
        ...
        all_models_results[f"DENSENET201 + SVM ({kernel})"] = {
            "y_true": test_labels,
            "y_pred": svm_pred,
            "y_prob": svm_prob
        }
        ...
        if svm_metrics['f1'] > best_hybrid_model_f1:
            ...
            best_hybrid_model_name = f"DENSENET201 + SVM ({kernel})"
    ...
    if best_hybrid_model_name:
        ...
        if densenet_f1 > best_hybrid_model_f1:
            print(f"Note: DENSENET201 (Optimized) still outperforms hybrid models with F1 score: {densenet_f1:.4f}")
        else:
            print(f"Hybrid model outperforms DENSENET201 (Optimized) (F1: {densenet_f1:.4f})")
    else:
